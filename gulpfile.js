...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/asp.netwebforms-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/asp.netwebforms-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore') {
                ...
                if (fs.existsSync('../asp.netwebforms-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/asp.netwebforms-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../asp.netwebforms-docs/` + changedFileName, `../../../gitlabRepo/asp.netwebforms-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/asp.netwebforms-docs/')) {
                            shelljs.cp('-rf', `../asp.netwebforms-docs/` + changedFileName, `../../../gitlabRepo/asp.netwebforms-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/asp.netwebforms-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/asp.netwebforms-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/asp.netwebforms-docs`);
